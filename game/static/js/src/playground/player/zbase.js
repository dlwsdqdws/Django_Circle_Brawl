class Player extends BallGameObject {
  constructor(
    ...
    if (this.character === "me") {
      ...
      this.fireball_img.src =
        "https://circle-brawl.oss-cn-hongkong.aliyuncs.com/fire_balll.png";

      // flash
      this.flash_coldtime = 5;
      this.flash_img = new Image();
      this.flash_img.src =
        "https://circle-brawl.oss-cn-hongkong.aliyuncs.com/blink.png";

      // shield
      this.shield_coldtime = 10;
      this.shield_img = new Image();
      this.shield_img.src =
        "https://circle-brawl.oss-cn-hongkong.aliyuncs.com/shield.png";
    }
  }
  ...
  start() {
    ...
    this.playground.notice_board.write(
      "Ready: " +
        this.playground.player_count +
        (this.playground.player_count == 1 ? " Player" : " Players")
    );
  ...
  update_move() {
    if (this.character === "robot" && this.spent_time > 5) {
      if (Math.random() * 200 < 1) {
        ...
        let tx =
          player.x + ((player.speed * this.vx * this.timedelta) / 1000) * 0.2;
        let ty =
          player.y + ((player.speed * this.vy * this.timedelta) / 1000) * 0.2;
  ...
  render_skill_coldtime() {
    ...
    if (this.flash_coldtime >= this.eps) {
      ...
      this.ctx.arc(
        ...
        r * scale,
        0 - Math.PI / 2,
        Math.PI * 2 * (1 - this.flash_coldtime / 5) - Math.PI / 2,
        true
      );
      this.ctx.lineTo(x * scale, y * scale);
      this.ctx.fillStyle = "rgba(169, 169, 169, 0.6)";
    ...
    if (this.shield_coldtime >= this.eps) {
      ...
      this.ctx.arc(
        ...
        r * scale,
        0 - Math.PI / 2,
        Math.PI * 2 * (1 - this.shield_coldtime / 10) - Math.PI / 2,
        true
      );
      this.ctx.lineTo(x * scale, y * scale);
      this.ctx.fillStyle = "rgba(169, 169, 169, 0.6)";
